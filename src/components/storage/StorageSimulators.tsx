      ...
      <div className="space-y-3 rounded-md border border-border bg-card p-3">
        ...
        <div className="flex gap-1">
          <button
            ...
            className="flex-1 rounded bg-muted px-2 py-1 text-[10px] hover:bg-accent"
          >
            Aggressive
          </button>
          <button
...
const steps = [
  {
    l: "Verify all tank isolation valves closed",
    why: "Open valves on startup mean uncontrolled flow into a cold/empty system.",
  },
  {
    l: "Check tank for free water (drain sump)",
    why: "Water bottoms can flash or freeze drain lines; must be removed first.",
  },
  {
    l: "Open inlet manifold valve slowly (10%)",
    why: "A snap-open shocks pipework and over-pressures gauges.",
  },
  {
    l: "Inspect all flanges for leaks",
    why: "Catch leaks at low flow, before full inventory is at risk.",
  },
  {
    l: "Gradually open tank inlet MOV-141 to 100%",
    why: "Throttling lets the level controller stabilise before full demand.",
  },
  {
    l: "Monitor tank level rise (max 5%/hr)",
    why: "Rapid filling generates static electricity — fire/explosion risk.",
  },
  {
    l: "Start crude transfer pump P-301",
    why: "Pump must only run after suction (tank) is verified full and vented.",
  },
  {
    l: "Verify metering station MS-301 reading",
    why: "Custody-transfer meter must read before any export begins.",
  },
  {
    l: "Open export isolation valve XV-301",
    why: "Final isolation only opens once metering and pump are confirmed stable.",
  },
  {
    l: "Confirm export flow at metering station",
    why: "Confirms physical flow matches what the meter reports.",
  },
  {
    l: "Check S-301/S-302 pressure within limits",
    why: "Spheres must be in band before tying them to the export header.",
  },
  {
    l: "Hand over to automatic LIC-301 control",
    why: "Only swap to AUTO once the loop is bumpless and the process is stable.",
  },
];
function Startup() {
  ...
  return (
    <div className="rounded-md border border-border bg-card p-4">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        Tap each step in the correct order. A wrong choice explains the danger.
      </p>
      ...
      {done && (
        <div
          ...
          style={{ borderColor: STG, background: `${STG}15`, color: STG }}
        >
          ✓ Tank farm online — crude flowing to export.
        </div>
      )}
    </div>
  );
...
const faults: { id: StorageFault; label: string; symptoms: string[]; explain: string }[] = [
  {
    ...
    explain:
      "The LT-301 transmitter has failed and is reporting a frozen value. LIC-301 sees no error, so LV-301 never opens — meanwhile real liquid keeps rising. Switch to MAN, verify with manual dip / radar gauge, replace the transmitter.",
  },
  {
    id: "psv_stuck",
    ...
    explain:
      "PSV-301 lifted but failed to reseat — gas keeps venting to flare/atmosphere. Isolate the sphere, depressurise safely, replace or overhaul the relief valve.",
  },
  {
    id: "pump_cav",
    ...
    explain:
      "Suction pressure has dropped below the liquid's vapour pressure — vapour bubbles form at the impeller and collapse, eroding it. Reduce demand, raise tank level / open suction valve fully, check for plugged strainer.",
  },
];
function FaultDiag() {
  ...
  return (
    <div className="space-y-3 rounded-md border border-border bg-card p-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={start}
          className="inline-flex items-center gap-1 rounded px-3 py-1 text-xs font-semibold text-white"
          style={{ background: STG }}
        >
      ...
      {active && (
        <>
          <div className="rounded-md border border-[var(--warning)]/40 bg-[var(--warning)]/10 p-3 text-xs">
            Fault active. Identify which device failed.
          </div>
          ...
          {result && (
            <div
              className={`rounded-md border p-3 text-sm ${result.ok ? "border-[var(--success)]/40 bg-[var(--success)]/10 text-[var(--success)]" : "border-[var(--danger)]/40 bg-[var(--danger)]/10 text-[var(--danger)]"}`}
            >
              <div className="mb-1 font-semibold">
                {result.ok ? "✓ Correct diagnosis" : "✗ Not quite"}
              </div>
...
function Inventory() {
  ...
  if (Math.abs(net) < 60 && s.tk301Level > 50 && s.tk301Level < 75)
    status = { color: STG, msg: "✅ Inventory balance optimal" };
  else if (net > 60)
    status = { color: "var(--warning)", msg: "⚠️ Tank filling rapidly — risk of overfill" };
      ...
      <div className="space-y-3 rounded-md border border-border bg-card p-4">
        ...
        <div>
          <div className="mb-0.5 flex justify-between font-mono text-xs">
            <span className="text-muted-foreground">Inlet flow</span>
            <span>{s.inletFlow.toFixed(0)} m³/h</span>
          </div>
          <input
            type="range"
            min={0}
            ...
            step={5}
            value={s.inletFlow}
            onChange={(e) => setInlet(+e.target.value)}
            className="w-full"
            style={{ accentColor: STG }}
          />
        </div>
        <div>
          <div className="mb-0.5 flex justify-between font-mono text-xs">
            <span className="text-muted-foreground">Export crude</span>
            <span>{s.exportCrude.toFixed(0)} m³/h</span>
          </div>
          <input
            type="range"
            min={0}
        ...
        <div className="rounded bg-background p-3 font-mono text-xs">
          ...
          <div className="flex justify-between">
            <span>Inlet</span>
            <span className="text-[var(--success)]">+{s.inletFlow.toFixed(0)} m³/h</span>
          </div>
          <div className="flex justify-between">
            <span>Export</span>
            <span className="text-[var(--danger)]">-{s.exportCrude.toFixed(0)} m³/h</span>
          </div>
          <div className="mt-1 flex justify-between border-t border-border pt-1">
            <span>Net change</span>
            <span style={{ color: net >= 0 ? STG : "var(--danger)" }}>
              {net >= 0 ? "+" : ""}
              {net.toFixed(0)} m³/h
            </span>
          </div>
        ...
        <div
          className="rounded p-3 text-center font-mono text-xs"
          style={{ background: `${status.color}20`, color: status.color }}
        >
          {status.msg}
        </div>
        <p className="text-[10px] text-muted-foreground">
          Goal: keep crude tanks 50–75 % capacity. Avoid overfill (&gt;95 %) and starvation (&lt;10
          %).
        </p>
      </div>
      <div className="rounded-md border border-border bg-card p-3">
        ...
        {[
          ...
          <div key={it.tag} className="mb-2">
            <div className="mb-0.5 flex justify-between font-mono text-[11px]">
              <span>{it.tag}</span>
              <span>
                {((it.v / 100) * it.c).toFixed(0)} m³ · {it.v.toFixed(0)}%
              </span>
            </div>
            <div className="h-2 w-full overflow-hidden rounded bg-muted">
              <div
                ...
                style={{
                  width: `${it.v}%`,
                  background: it.v > 85 ? "var(--danger)" : it.v < 15 ? "var(--warning)" : it.color,
                }}
              />
            </div>
          </div>
