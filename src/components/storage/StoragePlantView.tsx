  ...
  return (
    ...
    >
      ...
      >
        ...
        <text x="20" y="36" fill={STG} fontSize="20" className="scada-value" fontWeight="700">STOCKAGE ET EXPEDITION · STG-01</text>
        <text x={VBW - 20} y="36" fill="#9aa3b8" fontSize="14" className="scada-value" textAnchor="end">GAS &amp; OIL STORAGE TERMINAL</text>
        ...
        {/* Inlet source labels */}
        <text x="60" y="268" fill="#9aa3b8" fontSize="12" className="scada-value">FROM GAS</text>
        <text x="60" y="382" fill="#9aa3b8" fontSize="12" className="scada-value">FROM OIL</text>

        {/* === MAN-301 inlet manifold === */}
        <g onClick={sel("MAN-301")} className="cursor-pointer">
          <rect x="170" y="270" width="60" height="100" rx="3" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="1.5" />
          <rect x="178" y="286" width="44" height="16" fill="#0a0e1a" stroke="#3a4258" />
          <text x="200" y="298" textAnchor="middle" fill={STG} fontSize="10" className="scada-value" fontWeight="700">MAN-301</text>
          <text x="200" y="322" textAnchor="middle" fill="#e6e8ee" fontSize="11" className="scada-value">{s.inletFlow.toFixed(0)}</text>
          <text x="200" y="336" textAnchor="middle" fill="#9aa3b8" fontSize="9" className="scada-value">m³/h</text>
        </g>
        ...
        <g>
          ...
          <text x={1030} y={758} textAnchor="middle" fill={STG} fontSize="11" className="scada-value" fontWeight="700">LIC</text>
          <text x={1030} y={772} textAnchor="middle" fill={STG} fontSize="11" className="scada-value">301</text>
        </g>

        {/* Pump P-301 & Compressor K-301 */}
        <Pump x={1220} y={870} tag="P-301" subtitle="CRUDE" running={s.p301} onClick={sel("P-301")} />
        <Compressor x={1220} y={700} tag="K-301" subtitle="GAS" running={s.p302} press={s.s301Press} onClick={sel("P-302")} />
        ...
        {/* Export terminators */}
        <ExportBox x={1700} y={840} label="CRUDE" flow={`${s.exportCrude.toFixed(0)} m³/h`} color={OIL} />
        <ExportBox x={1700} y={670} label="GAS" flow={`${s.exportGas.toFixed(0)} m³/h`} color={GAS} />
        ...
        {/* Status panel (bottom) */}
        <g>
          <rect x="20" y={VBH - 70} width={VBW - 40} height="50" rx="4" fill="#0f1626" stroke="#232a3d" />
          ...
          <StatusDot x={430} y={VBH - 45} ok={s.s301Press < 18} label={`S-301 ${s.s301Press.toFixed(1)} bar`} />
          <StatusDot x={620} y={VBH - 45} ok={s.s302Press < 18} label={`S-302 ${s.s302Press.toFixed(1)} bar`} />
          <text x={830} y={VBH - 40} fill="#9aa3b8" fontSize="12" className="scada-value">Total inv:</text>
          <text x={935} y={VBH - 40} fill={STG} fontSize="13" className="scada-value">{(((s.tk301Level + s.tk302Level) / 100) * s.tankCapacity + ((s.s301Level + s.s302Level) / 100) * s.sphereCapacity).toFixed(0)} m³</text>
          <text x={830} y={VBH - 25} fill="#9aa3b8" fontSize="12" className="scada-value">Daily export:</text>
          <text x={955} y={VBH - 25} fill={STG} fontSize="13" className="scada-value">{s.dailyExport.toFixed(0)} m³</text>
        </g>

        {/* PID indicator box */}
        <g>
          <rect x={VBW - 360} y={VBH - 170} width="320" height="90" rx="4" fill="#0f1626" stroke="#232a3d" strokeWidth="1.5" />
          <text x={VBW - 350} y={VBH - 150} fill={STG} fontSize="12" className="scada-value" fontWeight="700">LIC-301 {s.licAuto ? "AUTO" : "MAN"}</text>
          <text x={VBW - 350} y={VBH - 132} fill="#e6e8ee" fontSize="11" className="scada-value">SP: {s.licSP.toFixed(0)}%   PV: {s.tk301Level.toFixed(1)}%</text>
          <text x={VBW - 350} y={VBH - 116} fill="#e6e8ee" fontSize="11" className="scada-value">e: {(s.tk301Level - s.licSP).toFixed(2)}   OUT(LV-301): {s.lv301.toFixed(0)}%</text>
          <text x={VBW - 350} y={VBH - 100} fill="#7d869c" fontSize="10" className="scada-value">Kp={s.kp.toFixed(2)} Ki={s.ki.toFixed(2)} Kd={s.kd.toFixed(2)}</text>
        </g>
      </svg>

      {/* Zoom controls (same style as Gas view) */}
      <div className="absolute right-2 top-2 z-10 flex flex-col gap-1">
        <button onClick={(e) => { e.stopPropagation(); setUserZoom((z) => Math.min(4, +(z + 0.2).toFixed(2))); }} className="grid h-9 w-9 place-items-center rounded-md border border-border bg-card/90 text-foreground hover:bg-muted backdrop-blur" aria-label="Zoom in"><ZoomIn className="h-4 w-4" /></button>
        <button onClick={(e) => { e.stopPropagation(); setUserZoom((z) => Math.max(0.4, +(z - 0.2).toFixed(2))); }} className="grid h-9 w-9 place-items-center rounded-md border border-border bg-card/90 text-foreground hover:bg-muted backdrop-blur" aria-label="Zoom out"><ZoomOut className="h-4 w-4" /></button>
        <button onClick={(e) => { e.stopPropagation(); reset(); }} className="grid h-9 w-9 place-items-center rounded-md border border-border bg-card/90 text-foreground hover:bg-muted backdrop-blur" aria-label="Fit"><Maximize2 className="h-4 w-4" /></button>
        <button onClick={(e) => { e.stopPropagation(); reset(); }} className="grid h-9 w-9 place-items-center rounded-md border border-border bg-card/90 text-foreground hover:bg-muted backdrop-blur" aria-label="Reset"><RotateCcw className="h-4 w-4" /></button>
...
function Pipe({ d, color, thin }: { d: string; color: string; thin?: boolean }) {
  ...
  return (
    <g>
      <path d={d} stroke="#1a2030" strokeWidth={outer} fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <path d={d} stroke="#3a4258" strokeWidth={mid} fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <path d={d} stroke={color} strokeWidth={inner} fill="none" strokeLinecap="round" strokeLinejoin="round" className="flow-pipe" opacity="0.85" />
    </g>
  );
}
...
function SensorDot({ x, y, type, id, onClick }: { x: number; y: number; type: SensorType; id: string; onClick?: (e: React.MouseEvent) => void }) {
  const colors: Record<SensorType, string> = { PT: "#3aa0ff", TT: "#ff5577", FT: "#00ff88", LT: "#ff9a3c" };
  return (
...
function Valve({ x, y, open, id, onClick }: { x: number; y: number; open: number; id: string; onClick?: (e: React.MouseEvent) => void }) {
  ...
  return (
    <g onClick={onClick} className="cursor-pointer">
      ...
      {/* bowtie body */}
      <polygon points={`${x - 22},${y - 18} ${x},${y} ${x - 22},${y + 18}`} fill="url(#metal-stg)" stroke="#1a2030" strokeWidth="1.5" />
      <polygon points={`${x + 22},${y - 18} ${x},${y} ${x + 22},${y + 18}`} fill="url(#metal-stg)" stroke="#1a2030" strokeWidth="1.5" />
...
function StatusDot({ x, y, ok, label }: { x: number; y: number; ok: boolean; label: string }) {
  ...
  return (
    <g>
      ...
      <text x={x + 14} y={y + 4} fill="#e6e8ee" fontSize="12" className="scada-value">{label}</text>
    </g>
  );
}
...
function Tank({ x, y, tag, level, temp, capacity, onClick }: { x: number; y: number; tag: string; level: number; temp: number; capacity: number; onClick?: (e: React.MouseEvent) => void }) {
  ...
  return (
    <g onClick={onClick} className="cursor-pointer">
      ...
      {/* roof */}
      <ellipse cx={x + W / 2} cy={y} rx={W / 2} ry="14" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="1.5" />
      {/* shell */}
      <rect x={x} y={y} width={W} height={H} fill="url(#vessel-stg)" stroke="#3a4258" strokeWidth="1.5" />
      {/* liquid */}
      <clipPath id={clipId}><rect x={x + 1} y={y + 1} width={W - 2} height={H - 2} /></clipPath>
      <g clipPath={`url(#${clipId})`}>
        <rect x={x} y={lvlY} width={W} height={H} fill="url(#liquid-stg)" opacity="0.9" />
        <line x1={x} y1={lvlY} x2={x + W} y2={lvlY} stroke="#ffe1b3" strokeWidth="2" opacity="0.8" />
      </g>
      {/* Sight glass */}
      ...
      {/* Label plate */}
      <rect x={x + W / 2 - 30} y={y + H / 2 - 12} width="60" height="24" fill="#0a0e1a" stroke="#3a4258" />
      <text x={x + W / 2} y={y + H / 2 + 4} textAnchor="middle" fill={STG} fontSize="12" className="scada-value" fontWeight="700">{tag}</text>
      {/* TI badge on roof */}
      <g>
        <rect x={x + W / 2 - 30} y={y - 36} width="60" height="20" rx="3" fill="#0a0e1a" stroke="#ff5577" />
        <text x={x + W / 2} y={y - 22} textAnchor="middle" fill="#ff5577" fontSize="11" className="scada-value">TI {temp.toFixed(0)}°C</text>
      </g>
      {/* Bottom label / status */}
      <text x={x + W / 2} y={y + H + 42} textAnchor="middle" fill="#9aa3b8" fontSize="12" className="scada-value">{tag} · {vol.toFixed(0)} m³</text>
      <text x={x + W / 2} y={y + H + 58} textAnchor="middle" fill={danger ? "var(--danger)" : "var(--success)"} fontSize="12" className="scada-value">L {level.toFixed(0)}%</text>
    </g>
  );
}
...
function Sphere({ x, y, tag, press, level, capacity, psv, onClick, onPsv }: { x: number; y: number; tag: string; press: number; level: number; capacity: number; psv: boolean; onClick?: (e: React.MouseEvent) => void; onPsv?: (e: React.MouseEvent) => void }) {
  ...
  return (
    <g>
      <g onClick={onClick} className="cursor-pointer">
        <clipPath id={clipId}><circle cx={x} cy={y} r={r - 2} /></clipPath>
        {/* body */}
        <circle cx={x} cy={y} r={r} fill="url(#vessel-stg)" stroke="#3a4258" strokeWidth="2" />
        <g clipPath={`url(#${clipId})`}>
          <rect x={x - r} y={fillY} width={r * 2} height={r * 2} fill="url(#gas-stg)" opacity="0.55" />
        </g>
        {/* highlight */}
        <ellipse cx={x - 28} cy={y - 30} rx="22" ry="10" fill="#ffffff" opacity="0.1" />
        {/* legs */}
        <line x1={x - r * 0.7} y1={y + r * 0.7} x2={x - r * 0.7} y2={y + r + 30} stroke="url(#metal-stg)" strokeWidth="4" />
        <line x1={x + r * 0.7} y1={y + r * 0.7} x2={x + r * 0.7} y2={y + r + 30} stroke="url(#metal-stg)" strokeWidth="4" />
        ...
        <text x={x} y={y + 5} textAnchor="middle" fill={STG} fontSize="13" className="scada-value" fontWeight="700">{tag}</text>
        {/* Press readout */}
        <text x={x} y={y - r - 22} textAnchor="middle" fill="#9aa3b8" fontSize="11" className="scada-value">Sphere {tag}</text>
        <text x={x} y={y - r - 6} textAnchor="middle" fill={pressDanger ? "var(--danger)" : "var(--success)"} fontSize="13" className="scada-value">P {press.toFixed(1)} bar</text>
        {/* Level readout */}
        <text x={x} y={y + r + 52} textAnchor="middle" fill={lvlDanger ? "var(--danger)" : "var(--success)"} fontSize="12" className="scada-value">L {level.toFixed(0)}% · {vol.toFixed(0)} m³</text>
      </g>
      {/* PSV */}
      <g onClick={onPsv} className={`cursor-pointer ${psv ? "animate-alarm" : ""}`}>
        <line x1={x} y1={y - r} x2={x} y2={y - r - 40} stroke="#3a4258" strokeWidth="3" />
        <rect x={x - 10} y={y - r - 52} width="20" height="14" fill={psv ? "var(--danger)" : "#3a4258"} stroke="#1a2030" />
        <text x={x + 32} y={y - r - 42} fill="#9aa3b8" fontSize="9" className="scada-value">PSV-30{tag === "S-301" ? "1" : "2"}</text>
      </g>
    </g>
  );
}

function Pump({ x, y, tag, subtitle, running, onClick }: { x: number; y: number; tag: string; subtitle: string; running: boolean; onClick?: (e: React.MouseEvent) => void }) {
  ...
  return (
    <g onClick={onClick} className="cursor-pointer">
      ...
      <rect x={x - 68} y={y - 22} width="42" height="44" rx="4" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="1.5" />
      {[0, 1, 2].map((i) => <line key={i} x1={x - 64} y1={y - 14 + i * 12} x2={x - 30} y2={y - 14 + i * 12} stroke="#1a2030" strokeWidth="1.5" />)}
      ...
      <g transform={`translate(${x} ${y})`}>
        <g>
          {running && <animateTransform attributeName="transform" type="rotate" from="0" to="360" dur="0.6s" repeatCount="indefinite" />}
          {[0, 1, 2, 3, 4].map((i) => {
      ...
      {/* Labels */}
      <text x={x} y={y + 56} textAnchor="middle" fill="#9aa3b8" fontSize="12" className="scada-value">{tag} · {subtitle}</text>
      <text x={x} y={y + 72} textAnchor="middle" fill={c} fontSize="12" className="scada-value">{running ? "RUNNING" : "STOPPED"}</text>
    </g>
  );
}

function Compressor({ x, y, tag, subtitle, running, press, onClick }: { x: number; y: number; tag: string; subtitle: string; running: boolean; press?: number; onClick?: (e: React.MouseEvent) => void }) {
  const c = running ? "var(--success)" : "var(--danger)";
  return (
    <g onClick={onClick} className="cursor-pointer">
      {/* Skid */}
      ...
      {/* Motor */}
      <rect x={x - 82} y={y - 34} width="50" height="70" rx="5" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="1.5" />
      {[0, 1, 2, 3, 4].map((i) => <line key={i} x1={x - 78} y1={y - 26 + i * 12} x2={x - 34} y2={y - 26 + i * 12} stroke="#1a2030" strokeWidth="1.5" />)}
      ...
      {/* Barrel casing */}
      <ellipse cx={x} cy={y} rx="42" ry="38" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="2" />
      <ellipse cx={x} cy={y} rx="42" ry="38" fill="none" stroke="#1a2030" strokeWidth="1" strokeDasharray="2 3" />
      <circle cx={x} cy={y} r="26" fill="#0a0e1a" stroke="#3a4258" strokeWidth="1.5" />
      {/* Impeller */}
      <g transform={`translate(${x} ${y})`}>
        <g>
          {running && <animateTransform attributeName="transform" type="rotate" from="0" to="360" dur="0.5s" repeatCount="indefinite" />}
      ...
      {/* Labels */}
      <text x={x} y={y + 60} textAnchor="middle" fill="#9aa3b8" fontSize="12" className="scada-value">{tag} · {subtitle}</text>
      {press !== undefined && (
        <text x={x} y={y + 76} textAnchor="middle" fill={c} fontSize="12" className="scada-value">{press.toFixed(1)} bar · {running ? "2950" : "0"} rpm</text>
      )}
    </g>
  );
}
...
function Meter({ x, y, tag, flow, onClick }: { x: number; y: number; tag: string; flow: number; onClick?: (e: React.MouseEvent) => void }) {
  return (
    <g onClick={onClick} className="cursor-pointer">
      <rect x={x - 42} y={y - 26} width="84" height="52" rx="3" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="1.5" />
      <rect x={x - 36} y={y - 20} width="72" height="24" fill="#0a0e1a" stroke="#3a4258" />
      <text x={x} y={y - 3} textAnchor="middle" fill="#00ff88" fontSize="14" className="scada-value" fontWeight="700">{flow.toFixed(0)}</text>
      <text x={x} y={y + 18} textAnchor="middle" fill="#9aa3b8" fontSize="10" className="scada-value">{tag}</text>
      <text x={x} y={y + 40} textAnchor="middle" fill="#7d869c" fontSize="9" className="scada-value">m³/h</text>
    </g>
  );
}

function ExportBox({ x, y, label, flow, color }: { x: number; y: number; label: string; flow: string; color: string }) {
  return (
    <g>
      <rect x={x} y={y} width="90" height="70" rx="3" fill="url(#metal-stg)" stroke="#3a4258" strokeWidth="1.5" />
      <rect x={x + 6} y={y + 8} width="78" height="20" fill="#0a0e1a" stroke="#3a4258" />
      <text x={x + 45} y={y + 22} textAnchor="middle" fill={color} fontSize="12" className="scada-value" fontWeight="700">{label} →</text>
      <text x={x + 45} y={y + 44} textAnchor="middle" fill="#e6e8ee" fontSize="12" className="scada-value">{flow}</text>
      <text x={x + 45} y={y + 60} textAnchor="middle" fill="#7d869c" fontSize="9" className="scada-value">EXPORT</text>
    </g>
  );
}
...
function DetailPanel({ id }: { id: string }) {
  ...
  const eq: Record<string, { title: string; rows: [string, string][]; design?: string; status?: string }> = {
    ...
    "P-302": { title: "Gas Transfer Compressor K-301", rows: [["Status", s.p302 ? "RUNNING" : "STOPPED"], ["Discharge flow", `${s.exportGas.toFixed(0)} m³/h`]], design: "Reciprocating · 200 m³/h" },
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="space-y-1">
          {panel.rows.map(([k, v]) => (
            <div key={k} className="flex justify-between border-b border-border/60 py-1 font-mono text-xs">
              <span className="text-muted-foreground">{k}</span>
              <span className="scada-value" style={{ color: "#e6e8ee" }}>{v}</span>
            </div>
          ))}
