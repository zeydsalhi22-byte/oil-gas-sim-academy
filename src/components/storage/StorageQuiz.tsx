    ...
    return (
      <div className="mx-auto max-w-md rounded-md border border-border bg-card p-6 text-center">
        <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
          Storage Quiz complete
        </div>
        <div className="mt-3 scada-value text-5xl" style={{ color: "#00aa44" }}>
          {pct}%
        </div>
        ...
        <button
          onClick={reset}
          className="mt-5 inline-flex items-center gap-2 rounded px-4 py-2 text-sm font-semibold text-white"
    ...
    <div className="mx-auto max-w-2xl space-y-3">
      ...
      <div className="h-1 w-full overflow-hidden rounded bg-muted">
        <div
          ...
          style={{ width: `${((i + 1) / bank.length) * 100}%`, background: "#00aa44" }}
        />
      </div>
      <div className="rounded-md border border-border bg-card p-5">
        ...
        {picked !== null && (
          <div className="mt-4 rounded-md border border-border bg-background p-3 text-xs text-muted-foreground">
            <span style={{ color: "#00aa44" }}>Explanation: </span>
            {q.explain}
          </div>
        )}
        {picked !== null && (
          <button
            onClick={next}
            className="mt-3 w-full rounded py-2 text-sm font-semibold text-white hover:brightness-110"
            style={{ background: "#00aa44" }}
          >
