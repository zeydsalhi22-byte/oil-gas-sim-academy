  ...
  return (
    <div className="grid h-full gap-3 lg:grid-cols-3">
      <div className="space-y-3 lg:col-span-2">
        <Kpis />
        <ChartCard
          title="Crude Flow (m³/h)"
          data={data}
          keys={[{ k: "crudeFlow", c: "#ff8a4d" }]}
        />
        <div className="grid gap-3 sm:grid-cols-2">
          <ChartCard
            title="BS&W % (target < 0.5)"
            data={data}
            keys={[{ k: "bsw", c: "#ffcc00" }]}
          />
          <ChartCard title="Tank Level %" data={data} keys={[{ k: "tankLevel", c: "#ff8a4d" }]} />
          <ChartCard
            title="HX Outlet Temp (°C)"
            data={data}
            keys={[{ k: "hxOut", c: "#ff5577" }]}
          />
          <ChartCard
            title="Pump Efficiency %"
            data={data}
            keys={[{ k: "pumpEff", c: "#00ff88" }]}
          />
        </div>
      </div>
      <Controllers />
    ...
    <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
      {items.map((it) => (
        <div key={it.l} className="rounded-md border border-border bg-card p-2">
          ...
          <div className="scada-value text-base text-primary">
            {it.v}
  ...
  return (
    <div className="space-y-3">
      <Loop
        ...
        min={20}
        max={80}
        onAuto={(v) => set({ lic201Auto: v })}
        onSP={(v) => set({ lic201SP: v })}
      />
      <Loop
        title="LIC-202 · Water Level"
        auto={s.lic202Auto}
        sp={s.lic202SP}
        pv={s.waterLevel}
        unit="%"
        min={15}
        max={60}
        onAuto={(v) => set({ lic202Auto: v })}
        onSP={(v) => set({ lic202SP: v })}
      />
      <Loop
        title="FIC-201 · Crude Flow"
        auto={s.fic201Auto}
        sp={s.fic201SP}
        pv={s.crudeFlow}
        unit="m³/h"
        min={50}
        max={230}
        onAuto={(v) => set({ fic201Auto: v })}
        onSP={(v) => set({ fic201SP: v })}
      />
      <Loop
        title="TIC-201 · HX Outlet Temp"
        auto={s.tic201Auto}
        sp={s.tic201SP}
        pv={s.hxCrudeOut}
        unit="°C"
        min={60}
        max={140}
        onAuto={(v) => set({ tic201Auto: v })}
        onSP={(v) => set({ tic201SP: v })}
      />
      ...
      <div className="rounded-md border border-border bg-card p-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => useOilSim.getState().startPump()}
            className="flex-1 rounded bg-[var(--success)]/20 px-3 py-2 text-xs text-[var(--success)]"
          >
            START
          </button>
          <button
            onClick={() => useOilSim.getState().stopPump()}
            className="flex-1 rounded bg-[var(--danger)]/20 px-3 py-2 text-xs text-[var(--danger)]"
          >
            STOP
          </button>
        </div>
  ...
  return (
    <div className="rounded-md border border-border bg-card p-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button
          onClick={() => onAuto(!auto)}
          className={`rounded px-2 py-0.5 font-mono text-[10px] ${auto ? "bg-[var(--success)]/20 text-[var(--success)]" : "bg-[var(--warning)]/20 text-[var(--warning)]"}`}
        >
          {auto ? "AUTO" : "MAN"}
        </button>
      </div>
