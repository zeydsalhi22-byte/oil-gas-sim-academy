...
import { gasQuiz, type Question } from "@/lib/sim/quiz";
import { oilQuiz } from "@/lib/sim/oilQuiz";
import { Check, X, RotateCcw } from "lucide-react";

export function OilQuiz() {
  const bank: Question[] = oilQuiz;
  ...
  if (done) {
    ...
    const rating =
      pct >= 90 ? "Excellent" : pct >= 70 ? "Good" : pct >= 50 ? "Pass" : "Needs review";
    return (
      <div className="mx-auto max-w-md rounded-md border border-border bg-card p-6 text-center">
        ...
        <div className="mt-1 text-sm text-muted-foreground">
          {score} / {bank.length} correct · {rating}
        </div>
        <button
          onClick={reset}
          className="mt-5 inline-flex items-center gap-2 rounded bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"
        >
    ...
    <div className="mx-auto max-w-2xl space-y-3">
      ...
      <div className="h-1 w-full overflow-hidden rounded bg-muted">
        <div
          ...
          style={{ width: `${((i + 1) / bank.length) * 100}%` }}
        />
      </div>
      <div className="rounded-md border border-border bg-card p-5">
          ...
          {q.options.map((o, idx) => {
            ...
            return (
              <button
                key={idx}
                onClick={() => choose(idx)}
                className={`flex w-full items-center justify-between rounded border p-3 text-left text-sm ${cls}`}
              >
                <span>{o}</span>
        ...
        {picked !== null && (
          <button
            onClick={next}
            className="mt-3 w-full rounded bg-primary py-2 text-sm font-semibold text-primary-foreground hover:brightness-110"
          >
