  ...
  return (
    ...
    >
      ...
      >
        ...
        <g onClick={() => select("well")} className="cursor-pointer">
          ...
          <circle
            ...
            cy="180"
            r="11"
            fill={colorFor(wellLvl)}
            className="pulse-glow"
            style={{ color: colorFor(wellLvl) }}
          />
          {/* hand wheel */}
        ...
        <g onClick={() => select("sep")} className="cursor-pointer">
          ...
          <path
            d="M 290 460 A 90 30 0 0 0 480 460 Z"
            fill="url(#vessel)"
            stroke="#3a4258"
            strokeWidth="1.5"
          />
          ...
          {/* Top elliptical head */}
          <path
            d="M 290 170 A 90 30 0 0 1 480 170 Z"
            fill="url(#vessel)"
            stroke="#3a4258"
            strokeWidth="1.5"
          />
          ...
          <clipPath id="sepClip">
            <rect x="291" y="171" width="188" height="289" />
          </clipPath>
          <g clipPath="url(#sepClip)">
            ...
            <line
              x1="290"
              y1={460 - (s.sepLevel / 100) * 280}
              x2="480"
              y2={460 - (s.sepLevel / 100) * 280}
              stroke="#ffe1b3"
              strokeWidth="2"
              opacity="0.8"
            />
          ...
          <rect
            x="455"
            y={440 - (s.sepLevel / 100) * 240}
            width="10"
            height={(s.sepLevel / 100) * 240}
            fill="#ff8800"
            opacity="0.9"
          />
          {/* Nozzles */}
          ...
          {[0, 1, 2, 3, 4, 5].map((i) => (
            <circle
              key={i}
              cx={385 + 14 * Math.cos((i * Math.PI) / 3)}
              cy={230 + 14 * Math.sin((i * Math.PI) / 3)}
              r="1.5"
              fill="#3a4258"
            />
          ))}
          ...
          <text
            ...
            y="580"
            textAnchor="middle"
            fill={colorFor(sepLvlA)}
            fontSize="14"
            className="scada-value"
          >
            L {s.sepLevel.toFixed(0)}% · P {s.sepPressure.toFixed(1)} bar
          </text>
        </g>
        ...
        <g onClick={() => select("comp")} className="cursor-pointer">
          ...
          <text
            ...
            y="385"
            textAnchor="middle"
            fill={colorFor(dischLvl)}
            fontSize="14"
            className="scada-value"
          >
            {s.discharge.toFixed(1)} bar · {s.rpm.toFixed(0)} rpm
          </text>
        </g>
        ...
        <g onClick={() => select("treat")} className="cursor-pointer">
          {/* Top head */}
          <path
            d="M 920 400 A 50 18 0 0 1 1020 400 Z"
            fill="url(#vessel)"
            stroke="#3a4258"
            strokeWidth="1.5"
          />
          ...
          />
          <path
            d="M 920 580 A 50 18 0 0 0 1020 580 Z"
            fill="url(#vessel)"
            stroke="#3a4258"
            strokeWidth="1.5"
          />
          ...
          <text
            ...
            className="scada-value"
          >
            {s.purity.toFixed(2)}% pure
          </text>
        </g>

        {/* LV liquid drain */}
        <Valve x={770} y={470} open={s.lvOpen} id="LV-101" onClick={() => select("lv")} />
        <text x={830} y={475} fill="#9aa3b8" fontSize="11" className="scada-value">
          → drain
        </text>
      </svg>
      ...
      <div className="absolute right-2 top-2 z-10 flex flex-col gap-1">
        ...
        <button
          onClick={() => setUserZoom((z) => Math.max(0.4, +(z - 0.2).toFixed(2)))}
          className="grid h-9 w-9 place-items-center rounded-md border border-border bg-card/90 text-foreground hover:bg-muted backdrop-blur"
          aria-label="Zoom out"
        >
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div className="mt-2 space-y-1 font-mono text-xs">{renderDetail(id, s)}</div>
      </motion.div>
    </AnimatePresence>
  );
}
...
function renderDetail(id: string, s: ReturnType<typeof useSim.getState>) {
  if (id === "well")
    return (
      <>
        <Row k="Wellhead Pressure" v={`${s.wellPressure.toFixed(1)} bar`} />
        <Row k="Temperature" v={`${s.wellTemp.toFixed(1)} °C`} />
        <Row k="Status" v="OPEN" c="var(--success)" />
      </>
    );
  if (id === "sep")
    return (
      <>
        <Row k="Level" v={`${s.sepLevel.toFixed(1)} %`} />
        <Row k="Pressure" v={`${s.sepPressure.toFixed(2)} bar`} />
        <Row k="Temp" v={`${s.sepTemp.toFixed(1)} °C`} />
        <Row k="Gas Out" v={`${s.gasFlow.toFixed(0)} m³/h`} />
        <Row k="Liq Out" v={`${s.liqFlow.toFixed(1)} m³/h`} />
      </>
    );
  if (id === "comp")
    return (
      <>
        <Row k="Suction" v={`${s.suction.toFixed(2)} bar`} />
        <Row k="Discharge" v={`${s.discharge.toFixed(2)} bar`} />
        <Row k="RPM" v={s.rpm.toFixed(0)} />
        <Row k="Temp" v={`${s.compTemp.toFixed(1)} °C`} />
        <Row k="Power" v={`${s.power.toFixed(0)} kW`} />
        <Row
          k="Status"
          v={s.compRunning ? "RUNNING" : "STOPPED"}
          c={s.compRunning ? "var(--success)" : "var(--danger)"}
        />
      </>
    );
  if (id === "treat")
    return (
      <>
        <Row k="Inlet" v="CH4 + H2O + CO2" />
        <Row k="Outlet Purity" v={`${s.purity.toFixed(2)} %`} c="var(--success)" />
        <Row k="Top Temp" v="38 °C" />
        <Row k="Bottom Temp" v="92 °C" />
      </>
    );
  if (id === "pv" || id === "fv" || id === "lv") {
    ...
    return (
      <>
        <Row k="Position" v={`${open.toFixed(1)} %`} />
        <Row k="Cv" v={cv} />
        <Row k="ΔP" v={`${dp.toFixed(2)} bar`} />
        <Row k="Flow" v={`${(cv * Math.sqrt(dp) * (open / 100)).toFixed(1)} m³/h`} />
      </>
    );
  }
  ...
  if (d) {
    ...
    return (
      <>
        <Row k="Process Value" v={v} />
        <Row k="Range" v={`${lo} – ${hi}`} />
        <Row k="Signal" v={`${mA(v, lo, hi).toFixed(2)} mA`} c="var(--primary)" />
        <Row
          k="LO / HI"
          v={`${(lo + (hi - lo) * 0.1).toFixed(0)} / ${(lo + (hi - lo) * 0.9).toFixed(0)}`}
        />
      </>
    );
  }
