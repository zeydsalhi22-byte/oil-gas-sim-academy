  ...
  return (
    <div className="grid h-full gap-3 lg:grid-cols-3">
      <div className="space-y-3 lg:col-span-2">
        <Kpis />
        <ChartCard
          title="Discharge Pressure (bar) — SP/PV"
          data={data}
          keys={[{ k: "discharge", c: "#ff6b00" }]}
        />
        <div className="grid gap-3 sm:grid-cols-2">
          <ChartCard
            title="Separator Level %"
            data={data}
            keys={[{ k: "sepLevel", c: "#ff9a3c" }]}
          />
    ...
    <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
      {items.map((it) => (
        <div key={it.l} className="rounded-md border border-border bg-card p-2">
          ...
          <div className="scada-value text-lg text-primary">
            {it.v}
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-border bg-card p-3">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => useSim.getState().startCompressor()}
            className="flex-1 rounded bg-[var(--success)]/20 px-3 py-2 text-xs text-[var(--success)]"
          >
            START
          </button>
          <button
            onClick={() => useSim.getState().stopCompressor()}
