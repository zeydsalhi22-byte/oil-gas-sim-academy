    ...
    <div className="grid gap-3 lg:grid-cols-[280px_1fr]">
      <div className="space-y-3 rounded-md border border-border bg-card p-3">
        ...
        <Slider
          ...
          max={10}
          step={0.1}
          hint="Proportional — higher = faster but oscillates"
        />
        <Slider
          label="Ki"
          value={Ki}
          ...
          max={100}
          step={1}
          hint="Integral — eliminates steady-state error"
        />
        <Slider
          label="Kd"
          value={Kd}
          ...
          max={1}
          step={0.01}
          hint="Derivative — damps fast changes"
        />
        ...
        <button
          onClick={apply}
          className="w-full rounded bg-primary py-2 text-sm font-semibold text-primary-foreground hover:brightness-110"
        >
          Apply to PIC-103
        </button>
      </div>
            ...
            <ResponsiveContainer>
              <LineChart data={sim.data}>
                ...
                <ReferenceLine
                  ...
                  stroke="#00ff88"
                  strokeDasharray="4 4"
                  label={{ value: "SP", fill: "#00ff88", fontSize: 10 }}
                />
                <Line
                  type="monotone"
                  dataKey="pv"
...
const startupSteps = [
  ...
  {
    l: "Energize anti-surge controller (open PV-101)",
    why: "Recycle line protects against surge at low flow.",
  },
  ...
  return (
    <div className="rounded-md border border-border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          onClick={() => {
            ...
            setError(null);
            setDone(false);
            useSim.getState().stopCompressor();
          }}
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        Click steps in the correct engineering order.
      </p>
...
const faults: FaultDef[] = [
  {
    ...
    symptoms: [
      {
        en: "Discharge pressure reading drops to 0 bar instantly",
        ar: "قراءة ضغط التفريغ تنخفض إلى 0 بار فجأة",
      },
      {
        en: "PID loop saturates the recycle valve trying to react",
        ar: "حلقة PID تشبع صمام الإرجاع محاولةً الاستجابة",
      },
      {
        en: "Other compressor readings (RPM, temp) remain normal",
        ar: "بقية قراءات الضاغط (السرعة، الحرارة) طبيعية",
      },
      {
        en: "No mechanical noise, no flow disturbance",
        ar: "لا ضجيج ميكانيكي ولا اضطراب في التدفق",
      },
    ],
    explain: {
      en: "A flat-zero reading on a single transmitter while the process is clearly running means the instrument signal is lost — typical 4-20 mA open circuit.",
      ar: "قراءة صفر ثابتة على ناقل واحد بينما العملية تعمل تعني فقدان إشارة الجهاز — انقطاع نموذجي في حلقة 4-20 mA.",
    },
  },
  {
    id: "valve_stuck",
    ...
    symptoms: [
      {
        en: "PV-101 position stays at 0% even when PID demands opening",
        ar: "وضعية PV-101 تبقى 0% رغم طلب PID فتحها",
      },
    ...
    explain: {
      en: "When PID output rises but valve position does not follow, the actuator or valve body is mechanically stuck.",
      ar: "عندما يرتفع مخرج PID دون أن تتبعه وضعية الصمام، يكون المحرك أو جسم الصمام عالقًا ميكانيكيًا.",
    },
  },
  {
    id: "leak",
    ...
    symptoms: [
      ...
      {
        en: "Gas outlet flow drops to roughly half",
        ar: "تدفق الغاز عند المخرج ينخفض إلى نحو النصف",
      },
      { en: "Suction pressure to the compressor drops", ar: "ضغط شفط الضاغط ينخفض" },
      {
        en: "Wellhead inflow unchanged — losses are between WH and K-101",
        ar: "تدفق رأس البئر لم يتغيّر — الفقد بين WH و K-101",
      },
    ],
    explain: {
      en: "Simultaneous low pressure and low flow downstream of an unchanged source point to a leak between the source and the measurement.",
      ar: "انخفاض الضغط والتدفق معًا أسفل مصدر ثابت يشير إلى تسرّب بين المصدر ونقطة القياس.",
    },
  },
  {
    id: "tt_fail",
    ...
    symptoms: [
      {
        en: "Compressor temperature jumps to an absurd value (>900°C)",
        ar: "حرارة الضاغط تقفز إلى قيمة غير منطقية (>900°C)",
      },
      { en: "Pressure, RPM and flow remain normal", ar: "الضغط والسرعة والتدفق تبقى طبيعية" },
      {
        en: "No vibration or sound change from K-101",
        ar: "لا تغيّر في الاهتزاز أو الصوت من K-101",
      },
      { en: "Triggers HIGH temperature alarm immediately", ar: "يطلق إنذار حرارة HIGH فورًا" },
    ],
    explain: {
      en: "An out-of-range temperature reading with no other process change is a transmitter failure — likely a broken RTD or burnt input.",
      ar: "قراءة حرارة خارج المدى مع عدم تغيّر باقي العملية تعني عطلًا في الناقل — غالبًا RTD مكسور أو دخل محترق.",
    },
  },
];
    ...
    <div className="space-y-3 rounded-md border border-border bg-card p-4">
      ...
      {active && (
        <>
          ...
          {submitted && active && (
            <div
              className={`space-y-1 rounded-md border p-3 text-sm ${correct ? "border-[var(--success)]/40 bg-[var(--success)]/10 text-[var(--success)]" : "border-[var(--danger)]/40 bg-[var(--danger)]/10 text-[var(--danger)]"}`}
            >
              ...
              <div className="text-xs opacity-90">
                <span className="font-mono uppercase tracking-wider">{t("explanation")}:</span>{" "}
                {active.explain[lang]}
              </div>
              {!correct && (
                <div className="text-xs opacity-90">
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-2">
      <div className="space-y-3 rounded-md border border-border bg-card p-4">
        ...
        <Slider
          label="Inlet flow (disturbance)"
          value={s.inletFlow}
          setValue={(v) => useSim.getState().setInletFlow(v)}
          min={200}
          max={3000}
          step={20}
          hint="Increase = higher load on the system"
        />
        <div className="flex items-center justify-between text-xs">
          <span>PID mode</span>
          <button
            onClick={() => useSim.getState().setPID({ pidAuto: !s.pidAuto })}
            className={`rounded px-3 py-1 font-mono ${s.pidAuto ? "bg-[var(--success)]/20 text-[var(--success)]" : "bg-[var(--warning)]/20 text-[var(--warning)]"}`}
          >
        ...
        {!s.pidAuto && (
          <Slider
            label="PV-101 (manual)"
            value={s.pvOpen}
            setValue={(v) => useSim.getState().setValve("pv", v)}
            min={0}
            max={100}
            step={1}
            hint="You control the recycle valve directly"
          />
        )}
        <Slider
          label="Setpoint (discharge)"
          value={s.setpoint}
          setValue={(v) => useSim.getState().setPID({ setpoint: v })}
          min={60}
          max={130}
          step={1}
          hint="Target pressure for PID"
        />
      </div>
