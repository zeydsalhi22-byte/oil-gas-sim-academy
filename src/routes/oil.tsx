...
export const Route = createFileRoute("/oil")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Offshore oil processing facility simulator — 3-phase separation, pumps, heat exchangers, dehydration, storage and SCADA training." },
      { property: "og:title", content: "Oil Sector — OilSim Academy" },
      { property: "og:description", content: "Interactive oil processing plant simulator with multi-loop PID, fault diagnosis, separator and heat exchanger design." },
    ],
  }),
  component: OilPage,
});
...
function OilPage() {
  ...
  useEffect(() => { setMounted(true); startOilTicker(); }, []);
  if (!mounted) return <div className="flex h-[100dvh] items-center justify-center bg-background font-mono text-sm text-muted-foreground">Loading facility…</div>;
  ...
  return (
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-background text-foreground">
      <header className="z-20 shrink-0 border-b border-border bg-background/95 backdrop-blur">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-2 px-3 py-2 sm:px-5">
          ...
          <div className="min-w-0">
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{lang === "ar" ? "قطاع النفط · OIL-01" : "Oil Sector · OIL-01"}</div>
          ...
          {([
            ...
            <button key={k} onClick={() => setTab(k as Tab)}
              className={`flex flex-1 items-center justify-center gap-2 border-b-2 px-3 py-2 text-sm font-medium transition ${
                tab === k ? "border-[#0080ff] text-[#0080ff]" : "border-transparent text-muted-foreground hover:text-foreground"
              }`}>
      ...
      <main className={`min-h-0 flex-1 ${tab === "plant" ? "flex flex-col overflow-hidden p-0" : "overflow-auto px-3 py-3 sm:px-5 sm:py-5"}`}>
        {tab === "plant" && <div className="min-h-0 flex-1"><OilPlantView /></div>}
      ...
      <nav className="z-20 grid shrink-0 grid-cols-4 border-t border-border bg-background/95 backdrop-blur sm:hidden">
        {([
          ...
          <button key={k} onClick={() => setTab(k as Tab)} className={`flex flex-col items-center gap-0.5 py-2 text-[10px] ${tab === k ? "text-[#0080ff]" : "text-muted-foreground"}`}>
