  ...
  head: () => ({
    meta: [
      { title: "OilSim Academy — Interactive Oil & Gas Training" },
      {
        name: "description",
        content:
          "Hands-on simulator for process control, PID tuning, sensors and SCADA for L3 Electronics/Instrumentation students.",
      },
      { property: "og:title", content: "OilSim Academy" },
      {
        property: "og:description",
        content: "Interactive oil & gas industrial simulation for engineering students.",
      },
    ],
  }),
  component: Home,
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5 sm:px-8">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={toggleLang}
            className="inline-flex items-center gap-1 rounded border border-border bg-card px-2 py-1 font-mono text-xs text-muted-foreground hover:text-foreground"
            aria-label="Toggle language"
          >
            <Languages className="h-3.5 w-3.5" />
            {lang === "en" ? "العربية" : "English"}
          </button>
          <a href="#sectors" className="text-sm text-muted-foreground hover:text-foreground">
            {t("sectors_anchor")}
          </a>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-12 pt-6 sm:px-8 sm:pt-14">
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <Link
              to="/gas"
              className="inline-flex items-center gap-2 rounded-md bg-primary px-5 py-3 font-semibold text-primary-foreground transition hover:brightness-110"
            >
              {t("enter_gas")} <ArrowRight className="h-4 w-4" />
            </Link>
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                <Gauge className="h-3.5 w-3.5" /> {t("real_pid")}
              </span>
              <span className="flex items-center gap-1">
                <Activity className="h-3.5 w-3.5" /> {t("live_trends")}
              </span>
            </div>
          </div>
        </motion.div>
